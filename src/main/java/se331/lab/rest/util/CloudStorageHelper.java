...
public class CloudStorageHelper {
    ...
    static{
        ...
        try{
            serviceAccount = new ClassPathResource("serviceKey.json").getInputStream();
            storage = StorageOptions.newBuilder().setCredentials(GoogleCredentials.fromStream(serviceAccount))
                    .setProjectId("imageupload-7075a").build().getService();
        }catch (IOException e){
            e.printStackTrace();
        }
    }
    ...
    public String uploadFile(MultipartFile filePart, final String bucketName)throws IOException{
        ...
        BlobInfo blobInfo = storage.create(BlobInfo
                ...
                .setAcl(new ArrayList<>(
                        Arrays.asList(Acl.of(Acl.User.ofAllUsers(), Acl.Role.READER)))).build(),
                os.toByteArray());
